...
import { photojournalEntries } from "../../../../../lib/schema";
import { desc } from "drizzle-orm";
import DeleteMemoryButton from "./DeleteMemoryButton";

export const dynamic = "force-dynamic";

export default async function AdminPhotojournalPage() {
  const entries = await db
    .select()
    .from(photojournalEntries)
    .orderBy(desc(photojournalEntries.date));

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl">Photojournal</h1>
        <Link
          href="/admin/photojournal/new"
      ...
      {entries.length === 0 ? (
        <p className="text-gray-500 text-sm">No local entries yet.</p>
      ) : (
          ...
          {entries.map((entry) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  href={`/admin/photojournal/${entry.slug}`}
                ...
                <DeleteMemoryButton slug={entry.slug} />
              </div>
            </li>
          ))}
        </ul>
