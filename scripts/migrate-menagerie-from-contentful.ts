/**
 * One-time migration script: Contentful photoEssay → Neon + Cloudinary
 *
 * Prerequisites:
 *   - DATABASE_URL set in .env.local
 *   - CONTENTFUL_* vars set in .env.local
 *   - CLOUDINARY_* vars set in .env.local
 *
 * Run:
 *   npx tsx --env-file=.env.local scripts/migrate-menagerie-from-contentful.ts
 *
...
async function main() {
  console.log("Fetching all photoEssay entries from Contentful...");
  ...
  for (const item of entries) {
    ...
    const { title, slug, date, opener, location, photos, previewPhoto } = fields;
    ...
    const [existing] = await db
      .select({ id: schema.menagerieEntries.id })
      .from(schema.menagerieEntries)
      .where(eq(schema.menagerieEntries.slug, slug));
    ...
    try {
      ...
      if (previewAsset?.fields?.file?.url) {
        ...
        const previewFilename = sanitizeFilename(
          `menagerie-${slug}-preview-${Date.now()}.${previewUrl.split("?")[0].split(".").pop() ?? "jpg"}`
        );
      ...
      const [entry] = await db
        .insert(schema.menagerieEntries)
        .values({
          title,
          slug,
          date,
      ...
      for (let i = 0; i < photoAssets.length; i++) {
        ...
        const filename = sanitizeFilename(
          `menagerie-${slug}-${String(i + 1).padStart(3, "0")}-${Date.now()}.${ext}`
        );
      ...
      if (blockValues.length > 0) {
        await db.insert(schema.menagerieBlocks).values(blockValues);
      }
